...
def display_inlier_outlier(cloud, ind):
    ...
    o3d.io.write_point_cloud("Output_Outlier.ply",inlier_cloud )
...
def statistical(pcd,vox,ne,sd):
    print("Statistical oulier removal")

    #cl, ind = voxel_down_pcd.remove_statistical_outlier(nb_neighbors=20,
    #                                                   std_ratio=2.0)
    pcd=o3d.io.read_point_cloud(input_path)
    voxel_down_pcd = pcd.voxel_down_sample(voxel_size=vox)
    cl, ind = voxel_down_pcd.remove_statistical_outlier(ne, sd)
    #down_pcd = down_pcd.select_down_sample(ind)
    display_inlier_outlier(voxel_down_pcd, ind)

#def radius(pcd,nb,rad):    
#    print("Radius oulier removal")
#    pcd=o3d.io.read_point_cloud(input_path)
#    voxel_down_pcd = pcd.voxel_down_sample(voxel_size=0.02)
#    cl, ind = voxel_down_pcd.remove_radius_outlier(nb_points=nb, radius=rad)
#    display_inlier_outlier(voxel_down_pcd, ind)	
...
if st.button("Statstical"):
    ...
    statistical(input_path,number,number1,number2)
...
#    radius(input_path,number1,number2)